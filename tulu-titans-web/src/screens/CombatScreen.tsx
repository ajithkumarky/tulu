...
const CombatScreen = () => {
  ...
  const useFiftyFifty = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/game/fifty-fifty', {
        ...
        body: JSON.stringify({ questionId: question.id, currentOptions: displayOptions, questionType: question.type }),
      });
      if (response.ok) {
  ...
  return (
    <CombatContainer>
      ...
      <QuestionCard>
        {question.type === 'image' && question.image_name && (
          <QuestionImagePrompt
        ...
        {showAnswerImage && question?.image_name && question.type !== 'image' && (
          <ImagePane>
...
const QuestionImagePrompt = styled.img`
  max-width: 90%;
  max-height: 260px;
  object-fit: contain;
  margin-bottom: 12px;
  border-radius: 12px;
  border: 3px solid #ffd700;
  box-shadow: 0 0 20px rgba(255, 215, 0, 0.4), 0 6px 20px rgba(0, 0, 0, 0.6);
`;
