...
const ADMIN_USERS = ['ajith'];
function isAdmin(username: string): boolean { return ADMIN_USERS.includes(username); }

// --- Column migration flags ---
let sentenceColumnsEnsured = false;
...
// Ensure vocabulary table has sentence review columns (idempotent, runs once per worker lifetime)
async function ensureSentenceColumns(db: D1Database): Promise<void> {
	if (sentenceColumnsEnsured) return;

	const columns = [
		"ALTER TABLE vocabulary ADD COLUMN sentence_status TEXT DEFAULT 'pending'",
		"ALTER TABLE vocabulary ADD COLUMN sentence_notes TEXT DEFAULT NULL",
	];
		...
		if (url.pathname.startsWith('/api')) {
			...
			if (url.pathname === '/api/game/question' && request.method === 'GET') {
				try {
					...
					// Pick question type with weighted probability
					const roll = Math.random();
					let questionType: 'translation' | 'reverse' | 'image';
					if (roll < 0.4) {
					...
					// For image type, check if vocab with images exists; fallback to translation
					let correctVocab: { id: number; tulu_word: string; english_translation: string; image_name: string; tulu_sentence_roman: string; sentence_english_translation: string } | null = null;

					if (questionType === 'image') {
						const imageCountResult = await env.DB.prepare("SELECT COUNT(*) as count FROM vocabulary WHERE image_name IS NOT NULL AND image_name != ''").first<{ count: number }>();
						const imageCount = imageCountResult ? imageCountResult.count : 0;
						if (imageCount > 0) {
							const randomOffset = Math.floor(Math.random() * imageCount);
							const rows = await env.DB.prepare(
								"SELECT id, tulu_word, english_translation, image_name, tulu_sentence_roman, sentence_english_translation FROM vocabulary WHERE image_name IS NOT NULL AND image_name != '' LIMIT 1 OFFSET ?"
							).bind(randomOffset).all();
							correctVocab = (rows.results as any[])[0] || null;
						}
						if (!correctVocab) {
							questionType = 'translation'; // fallback
						}
					}
					...
					// Determine answer field and question text based on type
					const answerField = questionType === 'translation' ? 'english_translation' : 'tulu_word';
					const correctAnswer = questionType === 'translation' ? correctVocab.english_translation : correctVocab.tulu_word;

					let questionText: string;
					if (questionType === 'translation') {
						questionText = `What is the English translation of "${correctVocab.tulu_word}"?`;
					} else if (questionType === 'reverse') {
						questionText = `What is the Tulu word for "${correctVocab.english_translation}"?`;
					} else {
						questionText = `What Tulu word does this image represent?`;
					}

					// Get 3 unique incorrect options from the same field
					const incorrectOptions: string[] = [];
					let attempts = 0;
					while (incorrectOptions.length < 3 && attempts < 20) {
						attempts++;
						const randomIncorrectOffset = Math.floor(Math.random() * totalVocabulary);
						const incorrectRows = await env.DB.prepare(
							`SELECT ${answerField} AS answer FROM vocabulary LIMIT 1 OFFSET ?`
						).bind(randomIncorrectOffset).all();
						const row = (incorrectRows.results as any[])[0];
						if (row && row.answer !== correctAnswer && !incorrectOptions.includes(row.answer)) {
							incorrectOptions.push(row.answer);
						}
					}
			...
			if (url.pathname === '/api/game/answer' && request.method === 'POST') {
				try {
					...
					const correctVocab = await env.DB.prepare(
						"SELECT tulu_word, english_translation FROM vocabulary WHERE id = ?"
					).bind(questionId).first<{ tulu_word: string; english_translation: string }>();

					if (!correctVocab) {
						return new Response(JSON.stringify({ error: "Invalid question ID" }), { status: 400, headers: apiHeaders });
					}

					const correctAnswer = (questionType === 'reverse' || questionType === 'image')
						? correctVocab.tulu_word
						: correctVocab.english_translation;
			...
			if (url.pathname === '/api/game/fifty-fifty' && request.method === 'POST') {
				try {
					const { questionId, currentOptions, questionType } = await request.json() as { questionId: number; currentOptions: string[]; questionType?: string };
					...
					const options = [correctAnswer, randomIncorrect].sort(() => Math.random() - 0.5);
			...
			if (url.pathname === '/api/admin/sentences' && request.method === 'GET') {
				try {
					const username = await getUsernameFromRequest(request, env.AUTH_SECRET);
					if (!username) {
						return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: apiHeaders });
					...
					if (!isAdmin(username)) {
						return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403, headers: apiHeaders });
					}

					await ensureSentenceColumns(env.DB);
					...
					let querySql = "SELECT id, tulu_word, english_translation, image_name, tulu_sentence_roman, sentence_english_translation, sentence_status, sentence_notes FROM vocabulary";
					const bindings: string[] = [];

					if (status && status !== 'all') {
						countSql += " WHERE sentence_status = ?";
						querySql += " WHERE sentence_status = ?";
					...
					const countStmt = bindings.length > 0
						? env.DB.prepare(countSql).bind(bindings[0])
						: env.DB.prepare(countSql);
					const countResult = await countStmt.first<{ total: number }>();
					const total = countResult?.total ?? 0;

					const queryBindings = [...bindings, pageSize.toString(), offset.toString()];
					let queryStmt;
					if (queryBindings.length === 3) {
						queryStmt = env.DB.prepare(querySql).bind(queryBindings[0], parseInt(queryBindings[1]), parseInt(queryBindings[2]));
					} else {
						queryStmt = env.DB.prepare(querySql).bind(pageSize, offset);
					}
					const { results } = await queryStmt.all();

					return Response.json({ sentences: results, total, page, pageSize }, { headers: apiHeaders });
				} catch (error) {
			...
			// --- Admin: Update sentence ---
			if (url.pathname.startsWith('/api/admin/sentences/') && request.method === 'POST') {
				try {
					const username = await getUsernameFromRequest(request, env.AUTH_SECRET);
					if (!username) {
						return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: apiHeaders });
					...
					if (isNaN(id)) {
						return new Response(JSON.stringify({ error: "Invalid ID" }), { status: 400, headers: apiHeaders });
					}

					const body = await request.json() as Record<string, string>;
					const allowedFields = ['tulu_sentence_roman', 'sentence_english_translation', 'sentence_status', 'sentence_notes'];
					...
					if (updates.length === 0) {
						return new Response(JSON.stringify({ error: "No valid fields to update" }), { status: 400, headers: apiHeaders });
					}

					const sql = `UPDATE vocabulary SET ${updates.join(', ')} WHERE id = ?`;
					await env.DB.prepare(sql).bind(...values, id).run();

					// Fetch updated row
					const updated = await env.DB.prepare(
						"SELECT id, tulu_word, english_translation, image_name, tulu_sentence_roman, sentence_english_translation, sentence_status, sentence_notes FROM vocabulary WHERE id = ?"
					).bind(id).first();

					return Response.json({ sentence: updated }, { headers: apiHeaders });
				} catch (error) {
